...
class TestStorageIntegrationPNG_SYNC(unittest.TestCase):
    def test_storage_image_upload_sync(self):
        ...
        br_remix_png_upload_result = hyper.storage.upload(
            name="remix", data=br_remix_png
        )

        br_remix_png_download_result = hyper.storage.download(name="remix")
        ...
        remove_result = hyper.storage.remove(name="remix")
